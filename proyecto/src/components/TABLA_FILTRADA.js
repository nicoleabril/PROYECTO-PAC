...
const seleccionarFila = (row) => {
    this.setState({ filaSeleccionada: row.id });
};
...
const editar_reforma = (id) => {
  window.location.href=`/editar_reforma/${id}/reformas/false`;
};
    ...
    return (
      <div>
        ...
        <table>
          ...
          <tbody>
            {table.getRowModel().rows.map((row) => (
              ...
              >
                {visualizar && (<td onClick={() => detalle_reforma(row.original.secuencial_reforma)} style={enlace}><i className="fa fa-eye" aria-hidden="true"></i></td>)}
                {verCambios && (<td onClick={() => ver_cambios(row.original.secuencial_reforma)} style={enlace}><i className="fa fa-eye" aria-hidden="true"></i></td>)}
                {editar && (<td onClick={() => editar_reforma(row.original.secuencial_reforma)} style={enlace}><i className="fa fa-pencil" aria-hidden="true"></i></td>)}
                {eliminar && (<td onClick={() => eliminar_reforma(row.original.secuencial_reforma)} style={enlace}><i className="fa fa-trash" aria-hidden="true"></i></td>)}
        ...
        <Modal isOpen={state.abierto} style={modalStyles}>
          ...
          <ModalFooter>
              <Button color="danger" onClick={abrirModal}>Sí, estoy seguro</Button>
