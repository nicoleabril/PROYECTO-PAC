...
import TablaFiltrada from '../components/TABLA_FILTRADA_RESOLUCION';
...
class Resoluciones_reformas extends Component {
    constructor(props) {
        super(props);
        this.state = {
            ...
            columns : [
                {
                    header: "Nro",
                    accessorKey: "secuencial_resolucion",
                    footer: "Nro",
                },
                {
                    header: "Codigo Resolución",
                    accessorKey: "codigo_resolucion",
                    footer: "Codigo Resolución",
                },
                {
                    header: "Fecha Solicitada",
                    accessorKey: "fch_solicitada",
                    footer: "Fecha Solicitada",
                },
                {
                    header: "Usuario Solicitante",
                    accessorKey: "usr_solicita",
                    footer: "Usuario Solicitante",
                },
                {
                    header: "Fecha Carga Documento",
                    accessorKey: "fch_carga_documento",
                    footer: "Fecha Carga Documento",
                },
                {
                    header: "Resolución",
                    accessorKey: "url_resolucion",
                    footer: "Resolución",
                },
                {
                    header: "Detalle Reforma",
                    accessorKey: "url_detalle_resol",
                    footer: "Detalle Reforma",
                },
            ]
        };
    }
    ...
    componentDidMount() {
        if (this.state.isLoggedIn) {
            this.obtenerResolucionesPendientes();
            this.obtenerResolucionesAplicadas();
        }
    }

    //190.154.254.187:5000

    obtenerResolucionesPendientes = async () => {
        const username = Cookies.get('usr');
        try {
            const response = await Axios.get(`http://190.154.254.187:5000/obtener_resoluciones/Pendiente`);
    ...
    obtenerResolucionesAplicadas = async () => {
        const username = Cookies.get('usr');
        try {
            const response = await Axios.get(`http://190.154.254.187:5000/obtener_resoluciones/Aplicada`);
    ...
    render() {
        ...
        return (
            <div>
                ...
                <body style={body}>
                    <div>
                        <h1>Resoluciones Reformas</h1>
                    </div>
                    <div style={encabezaGrupoForm}>
                        <table width="100%">
                            <tr>
                                <td style={{ width: '70%' }}><label style={etiqueta}>Resolución Pendiente</label></td>
                    ...
                    <div id="tablaFiltrada" className="form-group" style={grupoForm}>
                        {this.state.procesosSinRevision.length > 0 ? (
                            <TablaFiltrada data={this.state.procesosSinRevision} columns={this.state.columns} editar={true} />
                        ) : (
                    ...
                    <div style={encabezaGrupoForm}>
                        <table width="100%">
                            <tr>
                                <td style={{ width: '80%' }}><label style={etiqueta}>Resoluciones Aplicadas</label></td>
                                <td onClick={() => this.regresar_Inicio()} style={{ width: '10%' }}>Regresar</td>
                            </tr>
                        </table>
                    </div>
                    <div id="tablaFiltrada" className="form-group" style={grupoForm}>
                        {this.state.procesosConRevision.length > 0 ? (
                            <TablaFiltrada data={this.state.procesosConRevision} columns={this.state.columns} visualizar={true} />
                        ) : (
                            <p>No hay procesos disponibles.</p>
                        )}
                    </div>
