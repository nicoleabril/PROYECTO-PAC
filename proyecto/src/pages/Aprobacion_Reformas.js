...
function Aprobacion_Reformas() {
  ...
  const recargar_ventana = () => {
    ...
    const obtenerProcesosConRevision = async () => {
      ...
      try {
          ...
          const response = await Axios.get(`http://localhost:5000/obtener_reformas_a_aprobar/${rol}/${idDireccion}`);
  ...
  const aprobarReforma = async () => {
    if(filaSeleccionada!=null){
      try {
        const response = await Axios.post(`http://localhost:5000/cambiarEstadoReforma`, {estadoACambiar:'estado_aprobador', estadoNuevo:'Finalizado', secuencial_reforma:filaSeleccionada.secuencial_reforma});
        const response2 = await Axios.post(`http://localhost:5000/cambiarEstadoReforma`, {estadoACambiar:'estado_consolidador', estadoNuevo:'Iniciado', secuencial_reforma:filaSeleccionada.secuencial_reforma});
        ...
        setMensaje("Reforma aprobada correctamente")
        setSuccessVisible(true);
        setTimeout(() => setSuccessVisible(false), 3000);
        obtenerProcesosConRevision();
      } catch (error) {
        setMensaje("Ocurrió un problema al aprobar la reforma")
  ...
  const mandarReformaARevision = async () => {
    if(filaSeleccionada!=null){
      try {
        const response = await Axios.post(`http://localhost:5000/cambiarEstadoReforma`, {estadoACambiar:'estado_aprobador', estadoNuevo:'Pendiente', secuencial_reforma:filaSeleccionada.secuencial_reforma});
        const response2 = await Axios.post(`http://localhost:5000/cambiarEstadoReforma`, {estadoACambiar:'estado_revisor', estadoNuevo:'Iniciado', secuencial_reforma:filaSeleccionada.secuencial_reforma});
        console.log('Respuesta del servidor:', response.data);
        setProcesosConRevision(procesosConRevision.filter(proceso => proceso.secuencial_reforma !== filaSeleccionada.secuencial_reforma));
        setMensaje("Reforma colocada en revisión correctamente")
        setSuccessVisible(true);
        setTimeout(() => setSuccessVisible(false), 3000);
        obtenerProcesosConRevision();
      } catch (error) {
        setMensaje("Ocurrió un problema al colocar la reforma en revisión")
        setErrorVisible(true);
        setTimeout(() => setErrorVisible(false), 3000);
        console.error('Error al cambiar estado de Reforma:', error);
      }
  ...
  const showAprobacionConfirm = () => {
    if(filaSeleccionada !== null){
      confirm({
        title: '¿Está seguro de aprobar esta reforma?',
        icon: <ExclamationCircleFilled />,
        okText: 'Sí',
        okType: 'danger',
        cancelText: 'No',
  ...
  const showRevisarConfirm = () => {
    if(filaSeleccionada !== null){
      confirm({
        title: '¿Está seguro de mandar esta reforma a revisión?',
        icon: <ExclamationCircleFilled />,
        okText: 'Sí',
        okType: 'danger',
        cancelText: 'No',
        onOk() {
          mandarReformaARevision();
        },
        onCancel() {
          console.log('No borrar usuario');
        },
        ...
        <Row gutter={[24, 0]}>
          ...
          <Card bordered={false} className="criclebox tablespace mb-24" style={{ height: 'fit-content' }}>
            <div className="titleTable">
              <Title level={4} className="titleTable2" >Reformas por Aprobar</Title>
              <div className="botones" style={{ display: 'flex', gap: '10px' }}>
                ...
                <Button className="btnCrearProceso" type="primary" title="Solicitar Revisión" onClick={showRevisarConfirm}>
                    <span className="iconWrapper">
                      <HiDocumentMagnifyingGlass />
                    </span>
                </Button>
                <Button className="btnCrearProceso" type="primary" title="Aprobar Reforma" onClick={showAprobacionConfirm}>
                    <span className="iconWrapper">
                      <HiDocumentCheck />
                    </span>
                </Button>
