import {
  ...
  function Reformas() {
    ...
    const obtenerProcesosConRevision = async () => {
        ...
        try {
            const response = await Axios.get(`http://localhost:5000/obtenerReformas/estado_revisor/Iniciado/${username}`);
      ...
      useEffect(() => {
        ...
        const obtenerProcesosSinRevision = async () => {
            const username = Cookies.get('usr');
            try {
                const response = await Axios.get(`http://localhost:5000/obtenerReformas/estado_revisor/Pendiente/${username}`);
    ...
    const mandarReformaARevision = async () => {
        if(filaSeleccionada!=null){
          try {
            const response = await Axios.post(`http://localhost:5000/cambiarEstadoReforma`, {estadoACambiar:'estado_revisor', estadoNuevo:'Iniciado', secuencial_reforma:filaSeleccionada.secuencial_reforma});
            console.log('Respuesta del servidor:', response.data);
            setProcesosSinRevision(procesosSinRevision.filter(proceso => proceso.secuencial_reforma !== filaSeleccionada.secuencial_reforma));
    ...
    return (
              ...
              <div className="titleTable">
                ...
                <div className="botones" style={{ display: 'flex', gap: '10px' }}>
                  <NavLink to="/panel">
                    ...
                    <Button className="btnCrearProceso" type="primary" title="Revisar Reforma" onClick={showAprobacionConfirm}>
                      <span className="iconWrapper">
                        <HiDocumentCheck />
                      </span>
                    </Button>
        ...
        <Container className="contenedorTitleTable" fluid>
          <Row gutter={[24, 0]}>
            ...
            <Card bordered={false} className="criclebox tablespace mb-24" style={{ height: 'fit-content' }}>
              <div className="titleTable">
                <Title level={4} className="titleTable2" >Seguimiento de Mis Reformas</Title>
              </div>
              </Card>
            </Col>
          </Row>
