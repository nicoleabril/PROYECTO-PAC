...
function Tables() {
  ...
  useEffect(() => {
    ...
    const obtenerProcesosPreparatorios = async () => {
      ...
      const rol = Cookies.get('rol');
      try {
          const response = await Axios.get(`http://localhost:5000/obtener_procesos_preparatorios/${username}`);
  ...
  const replanificar_proceso = (id) => {
    localStorage.setItem('id', id);
    history.push('/replanificar-proceso');
  };

  const eliminar_reforma = (id) => {
    localStorage.setItem('id', id);
    history.push('/eliminar-procesos');
  };
  ...
  const columnas = [
    ...
    },
    {
      ...
      fixed: 'left',
      render: (_, registro) => (
        <Typography.Link onClick={() => eliminar_reforma(registro.pac_fase_preparatoria_pk)} >
    ...
    {
      title: '',
      key: 'fase_preparatoria',
      fixed: 'left',
      render: (_, registro) => (
        <Typography.Link onClick={() => showFaseConfirm(registro.pac_fase_preparatoria_pk)} >
          <div style={{ textAlign: 'center' }}>
            <a style={{ display: 'block' }}>Iniciar</a>
            <a style={{ display: 'block' }}>Fase Preparatoria</a>
          </div>
        </Typography.Link>
      ),
    ...
    {
      title: '',
      key: 'replanificar_proceso',
      fixed: 'left',
      render: (_, registro) => (
        <Typography.Link onClick={() => replanificar_proceso(registro.pac_fase_preparatoria_pk)} >
          <div style={{ textAlign: 'center' }}>
            <a style={{ display: 'block' }}>Replanificar</a>
            <a style={{ display: 'block' }}>Proceso</a>
          </div>
        </Typography.Link>
      ),
      width: 100, // Ajusta el valor según tus necesidades
    ...
    <div>
        ...
        <Row gutter={[24, 0]}>
          ...
          <Card bordered={false} className="criclebox tablespace mb-24" style={{ height: 'fit-content' }}>
            <div className="titleTable">
              <Title level={4} className="titleTable2" >Mis Procesos</Title>
              <div className="botones" style={{ display: 'flex', gap: '10px' }}>
                ...
                <NavLink to="/incluir-procesos">
                  <Button className="btnCrearProceso" type="primary" title="Incluir Proceso">
                    <span className="iconWrapper">
                      <FiPlus />
                    </span>
                  </Button>
      ...
      <Container className="contenedorTitleTable" fluid>
        <Row gutter={[24, 0]}>
          ...
          <Card bordered={false} className="criclebox tablespace mb-24" style={{ height: 'fit-content' }}>
            <div className="titleTable">
              <Title level={4} className="titleTable2" >Seguimiento de Mis Procesos</Title>
            </div>
            </Card>
          </Col>
        </Row>
