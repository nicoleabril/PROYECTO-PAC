import {
  ...
  function Control_Previo_Compras_Cat() {
    ...
    useEffect(() => {
        const obtenerProcesosSinRevision = async () => {
          const valor = 'CONTROL PREVIO COMPRAS';
          ...
          try {
              ...
              const response = await Axios.get(`http://localhost:5000/obtener_fase_preparatoria/${valorCodificado}/SI/${idDepartamento}/${rol}/${user}`);
    ...
    const editar_reforma = (id) => {
      ...
      localStorage.setItem('fase', 'Control Previo Compras');
      history.push('/subir-documentos-CAT');
    };
          ...
          <Row gutter={[24, 0]}>
            ...
            <Card bordered={false} className="criclebox tablespace mb-24" style={{ height: 'fit-content' }}>
              <div className="titleTable">
                <Title level={4} className="titleTable2" >Control Previo Compras</Title>
  ...
  export default Control_Previo_Compras_Cat;