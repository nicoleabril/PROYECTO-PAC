import {
  ...
  function Consolidar_Reformas() {
    ...
    const recargar_ventana = () => {
      ...
      const obtenerProcesosConRevision = async () => {
        ...
        try {
            const response = await Axios.get(`http://localhost:5000/obtener_reformas_a_consolidar/`);
    ...
    const consolidarReforma = async () => {
      if(filaSeleccionada!=null){
        try {
          const response = await Axios.post(`http://localhost:5000/cambiarEstadoReforma`, {estadoACambiar:'estado_consolidador', estadoNuevo:'Finalizado', secuencial_reforma:filaSeleccionada.secuencial_reforma});
          const response2 = await Axios.post(`http://localhost:5000/cambiarEstadoReforma`, {estadoACambiar:'estado_autorizador', estadoNuevo:'Iniciado', secuencial_reforma:filaSeleccionada.secuencial_reforma});
    ...
    const mandarReformaAAprobar = async () => {
      if(filaSeleccionada!=null){
        try {
          const response = await Axios.post(`http://localhost:5000/cambiarEstadoReforma`, {estadoACambiar:'estado_consolidador', estadoNuevo:'Pendiente', secuencial_reforma:filaSeleccionada.secuencial_reforma});
    ...
    const detalle_reforma = (id) => {
      ...
      localStorage.setItem('proceso', 'consolidar');
          ...
          <Row gutter={[24, 0]}>
            ...
            <Card bordered={false} className="criclebox tablespace mb-24" style={{ height: 'fit-content' }}>
              <div className="titleTable">
                <Title level={4} className="titleTable2" >Reformas por Consolidar</Title>
                <div className="botones" style={{ display: 'flex', gap: '10px' }}>
                  ...
                  <Button className="btnCrearProceso" type="primary" title="Consolidar Reforma" onClick={showConsolidarConfirm}>
                      <span className="iconWrapper">
                        <HiDocumentCheck />
                      </span>
                  </Button>
  ...
  export default Consolidar_Reformas;