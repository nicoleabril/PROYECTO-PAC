...
class Consolidacion_reformas extends Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    obtenerProcesosConRevision = async () => {
        ...
        try {
            const response = await Axios.get(`http://190.154.254.187:5000/obtenerReformas/estado_consolidador/Iniciado`);
    ...
    mandarPasoAnterior = async () => {
        if(this.state.filaSeleccionada!=null){
          try {
            const response = await Axios.post(`http://190.154.254.187:5000/cambiarEstadoReforma`, {estadoACambiar:'estado_aprobador', estadoNuevo:'Iniciado', secuencial_reforma:this.state.filaSeleccionada});
            const response_data = await Axios.post(`http://190.154.254.187:5000/cambiarEstadoReforma`, {estadoACambiar:'estado_consolidador', estadoNuevo:'Pendiente', secuencial_reforma:this.state.filaSeleccionada});
    ...
    consolidarReforma = async () => {
        if(this.state.filaSeleccionada!=null){
          try {
            const response = await Axios.post(`http://190.154.254.187:5000/cambiarEstadoReforma`, {estadoACambiar:'estado_consolidador', estadoNuevo:'Finalizado', secuencial_reforma:this.state.filaSeleccionada});
            const response2 = await Axios.post(`http://190.154.254.187:5000/cambiarEstadoReforma`, {estadoACambiar:'estado_autorizador', estadoNuevo:'Iniciado', secuencial_reforma:this.state.filaSeleccionada});
    ...
    render() {
        ...
        return (
            <div>
                ...
                <body style={body}>
                    <div>
                        <h1>Reformas por Consolidar</h1>
                        <div style={encabezaGrupoForm}>
                        <table width="100%">
                            <tr>
                                <td style={{ width: '70%' }}><label style={etiqueta}>Reformas por Consolidar</label></td>
                                <td onClick={() => this.consolidarReforma()} style={{ width: '10%' }}>Consolidar</td>
                                <td onClick={() => this.mandarPasoAnterior()} style={{ width: '10%' }}>Paso ant.</td>
...
export default Consolidacion_reformas;